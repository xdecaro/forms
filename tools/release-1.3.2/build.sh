...
tpl_line=next((line for line in builder.splitlines() if line.startswith('const templateDefs=')), '')
if not tpl_line: raise SystemExit('templateDefs line missing')
if "['receipt'" in tpl_line: raise SystemExit('Receipt template still in selectable defaults')
for token in ["['standard'","['minimal'","['institutional'","['card'","['compact'","['modern'","['elegant'","['custom'","['html'","['chatgpt'"]:
    if token not in tpl_line: raise SystemExit('Missing email template '+token)
